...
3. Stylometric heuristic #2: Selective punctuation usage
...
- reason_category (Enum String): An explicit categorical selection highlighting why the heuristic failed. Allowed values are:
1. BUSINESS_FORMAL: The text is a formal document (e.g., legal notification, HR self-rebuttal) where contractions and casual punctuation are intentionally omitted.
2. NON_NATIVE_SPEAKER: The author uses textbook-precise English grammar syntax that naturally mimics uniform AI generations.
3.COLLABORATIVE_DRAFT: The text was heavily human-edited or passed through a strict grammar linter (e.g., Grammarly) which stripped out human punctuation markers.
4. OTHER: The text contains unique content types (e.g., code blocks, logs, poetry, or highly customized technical formats) that disrupted the default stylometric baselines. (Requires a mandatory creator_comment)
- creator_comment (String): A short explanation adding granular context for the reviewer. If reason_category is set to OTHER, this field becomes strictly required by the backend schema validation.
...
The structured appeal metadata, including the selected reason_category and the user's optional or mandatory creator_comment.
...
The author's explicit appeal justification, grouping the categorical reason code and the descriptive comment side-by-side.
...
Context Provided to Claude: Provide the full Workflow Architecture ASCII Diagram, the complete Detection Signals section (Signals 1, 2, and 3), and the Uncertainty Representation & Calibration threshold tables.

Generation Prompt Objectives: Instruct Claude to implement the pure Python algorithmic functions for Stylometric Heuristic 1 (Contraction Density regular expression logic) and Stylometric Heuristic 2 (Punctuation Spark marker verification rule). Request the execution of the ensemble arbitration formula wrapper that multiplies the three incoming tracks by their explicit weights (0.50, 0.30, 0.20) to compute the unrounded final confidence metric.

Verification Gates: Pass explicit, contrasting test inputs through the pipeline loop and assert that the unrounded float outputs vary meaningfully. Verify that a raw text input intentionally written with hyper-formal sentences containing zero contractions and zero exclamation points yields a combined score moving well past 0.75, whereas a casual, shorthand draft containing conversational ellipses and contractions yields an unrounded score resting safely below 0.40.
